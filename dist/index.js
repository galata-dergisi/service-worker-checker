...
async function compareServiceWorkerCacheNames(repoPath) {
  try {
    ...
    const localFile = await external_fs_.promises.readFile(external_path_.join(repoPath, file), 'utf-8');
    ...
    core.info(`${ansi_styles.color.greenBright.open}Cache names are different.${ansi_styles.color.close}`);
  } catch (ex) {
    console.trace(ex);
...
async function main() {
  try {
    ...
    try {
      await external_fs_.promises.access(repoPath);
    } catch (ex) {
    ...
    for (const file of files) {
      ...
      core.info(`${ansi_styles.color.blueBright.open}Downloading ${url}${ansi_styles.color.close}`);
      const remoteFile = await getURL(url);

      if (localFile !== remoteFile) {
        const diff = diffChars(localFile, remoteFile);
        diff.forEach((part) => {
          const color = part.added ? 'green' :
            part.removed ? 'red' : 'grey';
          process.stderr.write(`${ansi_styles.color[color].open}${part.value}${ansi_styles.color.close}`);
        });

        process.stderr.write('\n\n');
        core.info(`${ansi_styles.color.yellowBright.open}${file} has changed.${ansi_styles.color.close}`);
        external_fs_.writeFileSync('local.txt', localFile);
        external_fs_.writeFileSync('remote.txt', remoteFile);
        return compareServiceWorkerCacheNames(repoPath);
      } else {
        core.info(`${ansi_styles.color.greenBright.open}${file} didn't change.${ansi_styles.color.close}`);
      }
    }

    core.info(`${ansi_styles.color.greenBright.open}There are no changes.${ansi_styles.color.close}`);
  } catch (ex) {
    console.trace(ex);
